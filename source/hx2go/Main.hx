...
class Main {
    ...
    public static function main() {
        ...
        var disableIncrementalCache = (args[8] == "1") ?? false;
        var lineDirectives = (args[9] != null) ? (args[9] == "1") : true;
        ...
        exec(absoluteInput, absoluteOutput, mainClass, singleFile, sourcelineComments, res, times, codegenVersion, disableIncrementalCache, lineDirectives);
    }
    ...
    public static function exec(input: String, output: String, mainClass: String, singleFile:Bool, sourcelineComments:Bool, res:haxe.ds.Map<String, Bytes>, timesBool:Bool = false, codegenVersion:String = "", disableIncrementalCache:Bool = false, lineDirectives:Bool = true): Void {
        ...
        generate(arc, output, mainClass, singleFile, sourcelineComments, res, times, codegenVersion, disableIncrementalCache, lineDirectives);

        final end = Sys.time();
        Sys.println('hx2go took ${Std.string(Math.round((end - start) * 100000) / 100)}ms');
    }

    public static function generate(archive: HxbArchive, absoluteOutput: String, mainClass: String, singleFile:Bool, sourcelineComments:Bool, res:haxe.ds.Map<String, Bytes>, times:hx2go.util.Times, codegenVersion:String, disableIncrementalCache:Bool = false, lineDirectives:Bool = true): Void {
        if (!FileSystem.exists(absoluteOutput)) {
            FileSystem.createDirectory(absoluteOutput);
        }

        var ctx = new hx2go.Context(archive, absoluteOutput, sourcelineComments, times, codegenVersion, disableIncrementalCache, lineDirectives);
