...
class RewriteArrayLength extends CompilerPass {
    ...
    public function execute(expr: HxbTypedExpr, frame: ContextFrame): Void {
        var o = switch expr.expr {
            case TField(e, FInstance({ name: 'Array', pack: [] }, [TDynamic(_) | TDynamicAny | TTypeParam(_) | TUnboundTypeParam(_)], { name: 'length' })): ExprHelper.createCallStatic(context, { name: 'HxDynamic', moduleName: 'HxDynamic', pack: ['go', 'hx2go'] }, 'getArrayLength', [e]);
