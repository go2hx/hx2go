...
class EnumWriter extends WriterImpl {
    ...
    public function writeEnum(e: HxbEnum): OutputBuffer {
        ...
        for (cs in e.constructors) {
            ...
            buf.add('func (this ${ctorName}) Hx_Field_enumParams() HxDynamicArray { return HxMakeArray[any]( ${ctorArgCount == 0 ? "" : ctorArgVars.map(v -> 'any(this.$v)').join(", ") + " " }).Dyn() }');
        ...
        buf.add('func (index int32, params HxDynamicArray) Hx_Obj_VTable_go_haxe__hxenumvalue__hxenumvalue {', 1);
        ...
        for (i in 0...e.names.length) {
            ...
            for (j in 0...counts[e.names[i]]) {
                args.push(dynamicTo(types[e.names[i]][j], ExprHelper.createUntyped('params.Get_Dyn($j)', [])));
            }
