...
class TypeWriter extends WriterImpl {
    ...
    public function writeHxbType(type: HxbType): OutputBuffer {
        ...
        var buf = new OutputBuffer(switch norm {
            ...
            case TType({ pack: ['go'], name: 'Tuple'}, [TAnon(anon)]): {
                var fields = anon.fields.copy();
            ...
            case TInst({ pack: [], name: 'Array' }, [TDynamic(_) | TDynamicAny]) | TAbstract({ pack: ["haxe", "ds"], name: "Vector" }, _): 'HxDynamicArray';
            case TInst({ pack: [], name: 'Array' }, params): 'HxArray[${writeHxbType(params[0])}]';
